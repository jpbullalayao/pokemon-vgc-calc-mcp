...
import type { CalculateDamageRequest, CalculateDamageResponse, Pokemon, Move, Field, Side } from './types.js';

const gen = Generations.get(9);

function createSmogonPokemon(pokemon: Pokemon): SmogonPokemon {
  const options: any = {
...
function createSmogonMove(move: Move): SmogonMove {
  const options: any = {};
...
function createSmogonSide(side?: Side): SmogonSide {
  const options: any = {};
...
function createSmogonField(field: Field): SmogonField {
  const options: any = {};
